...
export default async function BudgetPerformanceAnalysisPage() {
  ...
  try {
    ...
    items.forEach((item) => {
      // Safely convert Prisma Decimals/Numbers/Strings or nulls
      const estimated = Number(item.totalCostEstimate ?? 0);
      const disbursed = Number(item.performance?.amountDisbursed ?? 0);
      const cat = item.budgetCategory ? String(item.budgetCategory) : "OTHER";
      const comp = item.componentName ? String(item.componentName) : "Uncategorized";
    ...
    const totalEstimated = items.reduce(
      (acc, curr) => acc + Number(curr.totalCostEstimate ?? 0),
      0
    );
    const totalDisbursed = items.reduce(
      (acc, curr) => acc + Number(curr.performance?.amountDisbursed ?? 0),
      0
    );
    const overallExecutionRate =
      totalEstimated > 0 ? Math.round((totalDisbursed / totalEstimated) * 100) : 0;
    ...
    console.error("Error loading Budget Performance Analysis page:", error);
    return (
      <div className="max-w-4xl mx-auto p-8 bg-slate-900 border border-slate-800 rounded-2xl shadow-xl text-center space-y-4">
        <AlertTriangle className="w-10 h-10 text-amber-400 mx-auto" />
        <h2 className="text-lg font-bold text-white">Unable to Load Performance Analysis</h2>
        <p className="text-sm text-slate-400">
          An error occurred while aggregating workplan performance data. Please ensure your database connection is active and try refreshing the page.
        </p>
      </div>
    );
  }
